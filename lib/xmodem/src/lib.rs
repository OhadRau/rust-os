...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte_or_cancel(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let result = self.read_byte(false)?;
        if result == byte {
            Ok(result)
        } else if result == CAN {
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if buf.len() < 128 {
            return ioerr!(UnexpectedEof, "buf must be 128 bytes")
        }
        ...
        let transfer_kind = self.read_byte(true)?;
        if transfer_kind != SOH && transfer_kind != EOT {
            let _ = self.write_byte(CAN)?;
            return ioerr!(InvalidData, "first byte was not EOT or SOH") 
        } else if transfer_kind == EOT {
            let _ = self.write_byte(NAK)?;
            let _ = self.expect_byte(EOT, "expected EOT")?;
            let _ = self.write_byte(ACK)?;
            return Ok(0)
        }

        let _ = self.expect_byte_or_cancel(self.packet, "expected packet number")?;
        let _ = self.expect_byte_or_cancel(255 - self.packet, "expected one's complement of packet number")?;

        for i in 0..128 {
            // Can't receive a CAN during the packet transmission, so don't fail on CANs here
            buf[i] = self.read_byte(false)?;
        }

        let expected_checksum = self.read_byte(false)?;
        if expected_checksum != get_checksum(buf) {
            let _ = self.write_byte(NAK)?;
            return ioerr!(Interrupted, "checksum did not match expected value")
        }
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        if !self.started {
            let _ = self.expect_byte(NAK, "expected NAK to start transmission")?;
            self.started = true;
            (self.progress)(Progress::Started);
        }

        if buf.len() == 0 {
            let _ = self.write_byte(EOT)?;
            let _ = self.expect_byte(NAK, "expected NAK")?;
            let _ = self.write_byte(EOT)?;
            let _ = self.expect_byte(ACK, "expected ACK")?;
            return Ok(0)
        }

        let _ = self.write_byte(SOH)?;
        let _ = self.write_byte(self.packet)?;
        let _ = self.write_byte(255 - self.packet)?;

        for i in 0..128 {
            let _ = self.write_byte(buf[i])?;
        }

        let checksum = get_checksum(buf);
        let _ = self.write_byte(checksum)?;

        (self.progress)(Progress::Waiting);

        let reply = self.read_byte(true)?;
        if reply == ACK {
            self.packet = self.packet.wrapping_add(1);
            (self.progress)(Progress::Packet(128));
            Ok(128)
        } else if reply == NAK {
            ioerr!(Interrupted, "receiver indicated checksum mismatch")
        } else {
            ioerr!(InvalidData, "reply to packet was not NAK or ACK")
        }
    }
